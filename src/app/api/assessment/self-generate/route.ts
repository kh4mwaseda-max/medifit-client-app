...
export async function POST(req: NextRequest) {
  ...
  const isAuthenticated = cookieStore.get(`client_auth_${clientId}`)?.value === "1";
  ...
  const [bodyRes, trainingRes, mealRes] = await Promise.all([
    supabase.from("body_records").select("*").eq("client_id", clientId).order("recorded_at", { ascending: false }).limit(30),
    supabase.from("training_sessions").select("*, training_sets(*)").eq("client_id", clientId).order("session_date", { ascending: false }).limit(20),
    supabase.from("meal_records").select("*").eq("client_id", clientId).order("meal_date", { ascending: false }).limit(30),
  ]);
